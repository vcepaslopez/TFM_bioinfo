...
####### Filtering for female subjects and selected age groups (20-39 and 60-79)
####################################################################################################
...
####################################################################################################
##### 6. SAVING FILTERED METADATA (FEMALES 20-39 AND 60-79)
####################################################################################################

# Save the filtered metadata (with 20-39 and 60-79 years old subjects only) to a tab-delimited text file.
write.table(subjects_female_age, "GTEx_Analysis_v10_Annotations_SubjectPhenotypesDS_lung_women_20-39_60-79.txt", sep = "\t", row.names = FALSE, quote = FALSE)

####################################################################################################
##### 7. EXTRACT SUBJECT IDs FROM SAMPLE NAMES IN COUNT MATRIX
####################################################################################################
...
####################################################################################################
##### 8. FILTERING COUNT MATRIX TO MATCH SELECTED SUBJECTS
####################################################################################################

# Get vector of subject IDs to keep from the filtered metadata
subjects_to_keep <- subjects_female_age$SUBJID_dot
...
####################################################################################################
##### 9. SAVE FILTERED COUNT MATRIX
####################################################################################################

# Save the filtered RNA-seq count matrix to a new file
write.table(filtered_counts, "gene_reads_lung_female_20-39_60-79.gct", sep = "\t", row.names = FALSE, quote = FALSE)

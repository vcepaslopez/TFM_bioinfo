...
# It analyzes GTEx lung RNA-seq data from female subjects aged 20–39 vs. 60–79 and
# a Hardy scale 0 (Ventilator Case) using DESeq2.
...
# We analyze GTEx lung RNA-seq data comparing female subjects aged 20–39 vs. 60–79 and a Hardy scale 0.

# Load the count matrix (gene expression counts with genes as rows and samples as columns).
count_matrix <- read.delim("gene_reads_lung_female_20-39_60-79_Hardy0.gct", header = TRUE)
...
subjects_metadata <- read.delim("GTEx_Analysis_v10_Annotations_SubjectPhenotypesDS_lung_women_20-39_60-79_Hardy0.txt", 
                                header = TRUE, stringsAsFactors = FALSE)

# Check dimensions of the count matrix:
dim(count_matrix)
# [1] 59033   53
# 59033 genes x 53 columns (2 gene information columns + 51 samples)

# View data if needed:
# View(count_matrix)
# View(subjects_metadata)

# All samples are from females (SEX == 2), and only 20–39 and 60–79 age groups with a Hardy scale 0 are included.
...
# 3. Running statistical models and tests fir differential expression.
...
table(keep)
dim(dds)
# Of the initial 59033 genes, 36815 genes are retained and the remaining 22218 are discarded.
...
ggplot(pca_data_age, aes(PC1, PC2, color = AGE_GROUP)) +
  ...
  labs(
    color = "Age Group",
    title = "PCA by Age Group in Hardy Scale 0 Donors") +
  theme_classic() +
  theme(
...
# When we plot the PCA, we observe that the first principal component (X-axis) doesn't 
# really separate the samples by age group (20-39 and 60-79).
...
# We observe that 184 genes are overexpressed in the 60-79 age group vs. 20-39 age group (logfc = 0).
# We observe that 162 genes are overexpressed in the 60-79 age group vs. 20-39 age group (logfc = 0.5).
# We observe that 65 genes are overexpressed in the 60-79 age group vs. 20-39 age group (logfc = 1).
...
write.table(upregulated_genes, "upregulated_genes_60-79_vs_20-39_Hardy0_logfc0.txt", sep = "\t", quote = FALSE, row.names = TRUE)
...
table(res_trt$log2FoldChange < -logfc_threshold & res_trt$padj < padj_threshold)
# We observe that 53 genes are underexpressed in the 60-79 age group vs. 20-39 age group (logfc = 0).
# We observe that 43 genes are underexpressed in the 60-79 age group vs. 20-39 age group (logfc = 0.5).
# We observe that 32 genes are underexpressed in the 60-79 age group vs. 20-39 age group (logfc = 1).
...
# Save the table to a file
write.table(downregulated_genes, "downregulated_genes_60-79_vs_20-39_Hardy0_logfc0.txt", sep = "\t", quote = FALSE, row.names = TRUE)
...
# Now, we calculate as an example, how many of the overexpressed or underexpressed genes are expressed
# at least 4 times more or less in 60-79 subjects (FC = 4).
...
# We observe that 10 genes are expressed at least 4 times more in the 60-69 age group vs. 20-39 age group.

# We keep genes with a log2FC < 2 and an adjusted p-value < 0.1
table(res_trt$log2FoldChange < -logfc_threshold_fc4 & res_trt$padj < padj_threshold)
# We observe that 6 genes are expressed at least 4 times less in the 60-69 age group vs. 20-39 age group.
...
write.table(res_df, file = "rnaseq_women_60-79_vs_20-39_Hardy0.txt", row.names = FALSE, sep = "\t", quote = FALSE)
...
pdf("GO_functional_enrichment_women_60-79_vs_20-39_Hardy0_logfc0.5_padj0.1.pdf", 
    width = 6, # width of the PDf in inches 
    height = 7 # height of the PDF in inches
)
dotplot(fea_GO_BP, showCategory = 10, title="BP: Biological Process")
dotplot(fea_GO_MF, showCategory = 10, title="MF: Molecular Function")
dotplot(fea_GO_CC, showCategory = 10, title="CC: Cellular component")
...
write.table(x = data.frame(fea_GO_BP), 
            file = "deseq2_results_GO_BP_enrichment_women_60-79_vs_20-39_Hardy0_logfc0.5_padj0.1.txt", 
            row.names = FALSE, sep = "\t", quote = FALSE)

write.table(x = data.frame(fea_GO_MF), 
            file = "deseq2_results_GO_MF_enrichment_women_60-79_vs_20-39_Hardy0_logfc0.5_padj0.1.txt", 
            row.names = FALSE, sep = "\t", quote = FALSE)

write.table(x = data.frame(fea_GO_CC), 
            file = "deseq2_results_GO_CC_enrichment_women_60-79_vs_20-39_Hardy0_logfc0.5_padj0.1.txt", 
            row.names = FALSE, sep = "\t", quote = FALSE)
...
pdf("KEGG_functional_enrichment_women_60-79_vs_20-39_Hardy0_logfc0.5_padj0.1.pdf", 
    width = 6, # width of the PDf in inches 
    height = 7 # height of the PDF in inches
)
dotplot(fea_KEGG, showCategory = 10, title="KEGG Pathway Enrichment")
...
write.table(x = data.frame(fea_KEGG), 
            file = "deseq2_results_KEGG_enrichment_women_60-79_vs_20-39_Hardy0_logfc0.5_padj0.1.txt", 
            row.names = FALSE, sep = "\t", quote = FALSE)
...
pdf("volcano_plot_women_60-79_vs_20-39_Hardy0_logfc0.5_padj1e-10.pdf",
    width = 7, # width of the PDf in inches
    height = 6 # height of the PDF in inches
)
EnhancedVolcano(res_trt, # table of results
                lab = res_trt$ensembl, # name of the gene to print
